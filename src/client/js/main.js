...
function createItem(goodInformation){
    ...
    let innerElementsInformation = [
        ...
        ["div", "productState"],
        ["div", "productCost", goodInformation["price"] + " $"],
        ["button", "addPurchase", "Add to wishlist"],
    ]
    ...
    informationButton.setAttribute('data-tooltip', 'View product information.');
    ...
    let iconUrl;
    if (goodInformation["state"] === "Active")
...
function showGoods(dataGoods) {
    let categoryNames = ["Consoles", "Games", "Accessories"];
    clearCategoryTabs();
...
const increaseNumberOfProducts = id => {
    ...
    if (cart.get(id)['object']['count_of_product'] === cart.get(id)['count']) {
        document.getElementById('modal_notice_text').textContent =
            'Sorry! The quantity of this item is limited. It is not possible to add the item.';
        modalID = 'modal_notice'
        modalShow(document.getElementById(modalID));
        timeoutID = setTimeout(modalCloseInTime, 3000);
    } else {
        ...
        document.getElementById('total_num').textContent = total + ' usd.';
...
searchGoodsButton.addEventListener('submit',function(e){
    ...
    if (searchCategory === "Category")
        alert ("Choose a category for your search");
    else
...
function searchAndShowGoods (dataGoods) {
    ...
    if (isEmptyObject(searchAdditionalResults) === false) {
        let additionalResultTitleAttributes = ["div","categoryName", "Similar results"];
