...
function submitForm(e) {
    if (cart.size === 0) {
        alert("Your wishlist is empty. Please add at least one item to your cart before proceeding with your order.");
        e.preventDefault();
    } else {
        ...
        let type_of_delivery = ["Self-pickup. (Istanbul)", "Delivery", "Delivery ЕМС",
            "Delivery by Ptt", "Delivery by courier (only Istabul)"];
        let del = [];
        let delivery;
...
function ChangeState(dataGoods) {
    for (let item of cart.keys()) {
        if (dataGoods[Math.floor(item / 100) - 1][String(item)]["count_of_product"] === 0) {
            dataGoods[Math.floor(item / 100) - 1][String(item)]["state"] = "Expected arrival.";
        }
    }
    return dataGoods;
}
